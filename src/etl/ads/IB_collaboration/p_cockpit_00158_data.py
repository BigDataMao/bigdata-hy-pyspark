...
@log
def p_cockpit_00158_data(spark: SparkSession, busi_date: str):
    """
    溯源表模板_ib协同收入调整表-数据落地
    ...
    """
        v_sql := 'insert into CF_BUSIMG.T_COCKPIT_00158
    (month_id,
    ...
    select t.month_id,
           ...
           null
      from cf_busimg.T_COCKPIT_00125 t,
     ...
     where t.month_id = ' || v_month_id ||
        ...
        and a.account_code = ' || x.account_code || 'and c.fee_type = ''1004''
    ...
    df_result = spark.table("ddw.t_cockpit_00125").alias("t") \
        .filter(
        ...
        (col("c.fee_type") == lit("1004")) &
        (col("c.begin_date") <= lit(sys_date)) &
        (col("c.end_date") >= lit(sys_date)),
        "inner"
    ).join(
        spark.table("ddw.t_cockpit_00202").alias("d"),
        (col("d.fee_type") == lit("1006")) &
        (col("d.begin_date") <= lit(sys_date)) &
        (col("d.end_date") >= lit(sys_date)),
        "inner"
    ).groupBy(
        col("t.month_id"),
        ...
        when(
            ...
            sum(col("t.ibxt_interest_clear_income"))
        ).when(
            col("a.account_code") == "6021",  # 手续费及佣金收入
            sum(col("t.ibxt_income")) / (lit(1) + coalesce(col("c.para_value"), lit(0)))
        ).when(
            col("a.account_code") == "6111",  # 投资收益
            sum(col("t.ibxt_market_reduct_income")) / (lit(1) + coalesce(col("c.para_value"), lit(0)))
        ).when(
            col("a.account_code") == "6403",  # 税金及附加
            sum(
                col("t.ibxt_income") +
                col("t.ibxt_market_reduct_income")
    ...
    return_to_hive(
        spark=spark,
        df_result=df_result,
        target_table="ddw.t_cockpit_00158",
