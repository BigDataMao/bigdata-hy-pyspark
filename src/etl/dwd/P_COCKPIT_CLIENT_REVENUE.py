...
def p_cockpit_client_revenue(spark: SparkSession, busi_date: str):
    ...
    df_tmp_5 = df_tmp_4.alias("t").join(
        ...
        sum("t.interest_income" / (1 + col("c.para_value"))).alias("accrued_interest_after_tax"),
    ...
    df_tmp_6 = spark.table("ods.t_ds_adm_investor_value").alias("t") \
        ...
        round(sum("t.subsistence_fee_amt" / (1 + col("c.para_value"))), 2).alias("remain_transfee_after_tax"),
        ...
        round(sum("t.i_int_amt" / (1 + col("c.para_value"))), 2).alias("client_interest_after_tax"),
        ...
        round(sum("t.oth_amt" / (1 + col("c.para_value"))), 2).alias("other_income_after_tax"),
    ...
    df_tmp_8 = spark.table("ods.t_ds_adm_investor_value").alias("a") \
        ...
        round(sum("a.broker_amt" / (1 + col("d.para_value"))), 2).alias("cs_person_rebate_after_tax"),
        ...
        round(sum("a.broker_eret_amt" / (1 + col("d.para_value"))), 2).alias("cs_person_ret_after_tax"),
        ...
        round(sum("a.broker_int_amt" / (1 + col("d.para_value"))), 2).alias("cs_person_interest_after_tax"),
        ...
        round(sum("a.i_oth_amt" / (1 + col("d.para_value"))), 2).alias("other_pay_after_tax"),  # 其他支出_不含税
        round(sum("a.i_oth_amt" * col("e.para_value")), 2).alias("other_pay_add_tax"),  # 其他支出增值税附加税
        round(sum("a.i_oth_amt" * col("f.para_value")), 2).alias("other_pay_risk_fund")  # 其他支出风险金
    ).fillna(0)
