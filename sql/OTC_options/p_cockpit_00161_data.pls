create or replace procedure cf_busimg.p_cockpit_00161_data(i_busi_date   in varchar2,
  ...
  -- copyright              wolf 1.0
  -- func_create_begin
  -- func_id                p_cockpit_00161_data
  -- version                1.0
  -- func_name
  -- func_remark            溯源表模板_场外期权清算台账-数据落地
  ...
  --固定变量
  ---------------------------------------------------------------------------------------
  v_op_object  varchar2(50) default 'P_COCKPIT_00161_DATA'; -- '操作对象';
...
begin
  ...
  delete from CF_BUSIMG.T_COCKPIT_00161 t where t.month_id = v_month_id;
  commit;
  ...
  for x in c_cockpit_account loop
    ...
    v_sql := 'insert into CF_BUSIMG.T_COCKPIT_00161
    (month_id,
    ...
    select t.month_id,
           ...
           null
      from cf_busimg.T_COCKPIT_00208 t,
